...
CFG["SUMMARY"] = f"{EXP_NAME}: model:efficientnetb6, grid img size 256, img size 512, layer change. average bef aft layers"
...
def init_logger(log_file=os.path.join(CFG["OUTPUT_DIR"], 'infercheck.log')):
    """Output Log."""
    ...
    for fold in CFG["folds"]:
        ...
        for slice_idx, surface_list in enumerate(CFG["SURFACE_LIST"]):
            ...
            for slide_pos in CFG["slide_pos_list"]:
                ...
                for tta in tta_list:
                    print(f"tta = {tta}")
                    tta_transforms = get_tta_aug(tta)
                    valid_dataset = VCID_Dataset(CFG, valid_dirs, surface_list, surface_volumes, slide_pos, mode="valid", transform=tta_transforms)
