...
CFG["SUMMARY"] = "ex048: subtask-class. model:efficientnetb6, img size 512, not channel shuffle, bcewithlogits weight 0.3-0.3"
...
def train_fn(train_loader, model, criterion, epoch ,optimizer, scheduler):
    ...
    for batch_idx, (images, targets_mask, targets_class, _) in enumerate(train_loader):
        ...
        assert preds_mask is not None, f"preds_mask is None, {preds_mask}, {images}, {targets_mask}, {targets_class}"
        assert preds_class is not None, f"preds_class is None, {preds_class}, {images}, {targets_mask}, {targets_class}"
        
#         loss = criterion(preds, targets)
        loss = criterion(preds_class, targets_class, preds_mask, targets_mask)
        preds_mask = torch.sigmoid(preds_mask)
        assert loss is not None, f"loss is None, {loss}, {preds_class}, {preds_mask}, {targets_mask}, {targets_class}"
