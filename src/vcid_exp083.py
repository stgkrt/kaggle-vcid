...
CFG["SUMMARY"] = f"{EXP_NAME}: PSPnet model:resnext50_32x4, grid img size 512, img size 512 augment P is smaller"
...
MODEL
"""
class ModifiedPSPNet(nn.Module):
    def __init__(self, CFG_):
        super().__init__()
        self.conv = nn.Conv2d(CFG_["inp_channels"], 3, kernel_size=1)
        self.PSP = smp.PSPNet( encoder_name=CFG_["model_name"], 
                                encoder_weights="imagenet", 
                                classes=CFG_["out_channels"], 
                            )
    def forward(self, x):
        x = self.conv(x)
        x = self.PSP(x)
...
# x = torch.rand((1, CFG["inp_channels"], 512, 512)).float()
# print(x.shape)
# output = model(x)
# print(output.shape)
