...
CFG["SUMMARY"] = "ex059: subtask-class. model:efficientnetb6, img size 512, not channel shuffle, bcewithlogits weight 0.2-0.2, mask-class=0.8:0.2, Cycliclr, lr=1e-3"
...
def logging_metrics_epoch(CFG, fold, epoch, slice_idx,train_loss_avg, valid_loss_avg, score, threshold, auc_score, auc_class):
    wandb.log({f"train/fold{fold}": train_loss_avg,
                ...
                f"auc_class/fold{fold}":auc_class,
...
BCE_Mask = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor([0.2]).cuda())
BCE_Class = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor([0.2]).cuda())
...
def train_fn(train_loader, model, criterion, epoch ,optimizer, scheduler):
    ...
    losses = AverageMeter()
    start = end = time.time()
    for batch_idx, (images, targets_mask, targets_class, _) in enumerate(train_loader):
...
#         loss = criterion(preds, targets)
        ...
        assert loss is not None, f"loss is None, {loss}, {preds_class}, {preds_mask}, {targets_class}, {targets_mask}"
...
def concat_grid_img(img_list, label_list, grid_idx_list, valid_dir_list, CFG, slide_pos=[0,0]):
...
def training_loop(CFG):
    ...
    for fold in CFG["folds"]:
        ...
        valid_img_slice = []
#         criterion = BCE_ClassMask()
        optimizer = AdamW(model.parameters(), lr=CFG["base_lr"], weight_decay=CFG["weight_decay"], amsgrad=False)
        ...
        # training
        best_score, best_auc, best_valloss, best_auc_valloss = -np.inf, -np.inf, np.inf, np.inf
        best_auc_class = -np.inf
        best_threshold = -1
        start_time = time.time()
        best_epoch, best_auc_epoch, best_aucclass_epoch = -1, -1, -1
        ...
        for slice_idx, surface_list in enumerate(CFG["SURFACE_LIST"]):
            ...
            for epoch in range(1, CFG["n_epoch"] + 1):
                ...
                valid_targets, valid_preds, valid_targets_class, valid_preds_class, valid_grid_idx, valid_loss_avg = valid_fn(model, valid_loader, criterion)
                ...
                auc_class = roc_auc_score(valid_targets_class, valid_preds_class)
                ...
                if not CFG["DEBUG"]:
                    logging_metrics_epoch(CFG, fold, epoch, slice_idx, train_loss_avg, valid_loss_avg, valid_score, valid_threshold, auc, auc_class)
                scheduler.step()
                ...
                if auc_class > best_auc_class:
                    best_auc_class = auc_class
                    best_aucclass_epoch = epochs_
                    model_name = CFG["model_name"]
                    model_path = os.path.join(CFG["OUTPUT_DIR"], f'{model_name}_aucclass_fold{fold}.pth')
                    torch.save(model.state_dict(), model_path) 
                    LOGGER.info(f'Epoch {epochs_} - Save Best AUC CLASS: {best_auc_class:.4f}. Model is saved.')
                    # save oof
                    save_and_plot_oof("aucclass", fold, slice_idx, valid_preds_img, valid_targets_img, valid_preds_binary, CFG)
        ...
        LOGGER.info(f'[fold{fold}] BEST AUC CLASS Epoch {best_aucclass_epoch} - Save Best Score:{best_auc_class:.4f}.')
    ...
    for fold in CFG["folds"]:
        ...
        for slice_idx, surface_list in enumerate(CFG["SURFACE_LIST"]):
            ...
            for slide_pos in CFG["slide_pos_list"]:
                ...
                # target, predをconcatして元のサイズに戻す
                valid_preds_img, valid_targets_img  = concat_grid_img(valid_preds, valid_targets, valid_grid_idx, valid_dirs, CFG, slide_pos)
                valid_score, valid_threshold, auc, dice_list = calc_cv(valid_targets_img, valid_preds_img)
                valid_preds_binary = (valid_preds_img > valid_threshold).astype(np.uint8)
                ...
                elapsed = time.time() - start_time
                LOGGER.info(f"\t score:{valid_score:.4f}(th={valid_threshold:3f}), auc={auc:4f}, auc class={auc_class:4f} ::: time:{elapsed:.2f}s")
                # valid_img_slice.append(valid_preds_img)
