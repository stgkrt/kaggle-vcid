...
CFG["SUMMARY"] = f"{EXP_NAME}: model:efficientnetb6, pseudo labeling check"
...
class VCID_Dataset(Dataset):
    def __init__(self, CFG, data_dir_list, surface_list, surface_volumes=None, slide_pos=[0,0], mode="train", transform=None, oof_img_path=None):
        # get config
        self.mode = mode
        ...
        elif self.mode == "pseudo": self.DATADIR = CFG["TRAIN_DIR"]
        self.data_dir_list = data_dir_list
        self.surface_list = surface_list
        self.slide_pos = slide_pos
        self.transform = transform
        self.oof_img_path = oof_img_path
        if self.mode == "pseudo":
            self.threshold = CFG["pseudo_threshold"]
        ...
        elif self.mode=="pseudo" and self.oof_img_path is not None:
            label = cv2.imread(self.oof_img_path, cv2.IMREAD_GRAYSCALE)
            label = label.reshape(label.shape[0], label.shape[1], 1)
            label = (label >= self.threshold*255).astype(np.uint8)*255
            cv2.imwrite(os.path.join(CFG["OUTPUT_DIR"], "imgs", "check_pseudo_label_indataset.png"), label)
            self.labels = [label]
    ...
    def filter_pseudo_grid(self):
        """ get grid indice which pred > threshold by all grid indices""" 
        grid_indices_all = []
        for img, grid_indices in zip(self.labels, self.grid_indices):
            grid_indices_copy = grid_indices.copy()
            for grid_idx in grid_indices:
                img_grid = self.get_grid_img(img, grid_idx)
                if img_grid.sum() == 0:
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "test":
            ...
            return img, grid_idx
        elif self.mode == "train" or self.mode=="valid" or self.mode=="pseudo":
...
#     img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
#     label_img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)/255.
#     img = img.reshape(img.shape[0], img.shape[1], 1)
#     pred_img = np.zeros_like(img).astype(np.float32)
#     for img_idx, grid_idx in enumerate(grid_idx_list):
#         img_ = img_list[img_idx]
...
def pseudo_training_loop(CFG):
    ...
    for fold in CFG["folds"]:
        ...
        # set model & learning fn
        model = SegModel(CFG)
        model_path = os.path.join(CFG["OUTPUT_DIR"], "pretrained_weights", f'{CFG["model_name"]}_auc_fold{fold}.pth')
        ...
        for slice_idx, surface_list in enumerate(CFG["SURFACE_LIST"]):
            LOGGER.info(f"surface_list: {surface_list}")
            # separate train/valid data 
            pseudo_dirs = CFG["VALID_DIR_LIST"][fold]
            # oof_img_path = os.path.join(CFG["OUTPUT_DIR"], "imgs", f"fold{fold}_auc_slice{slice_idx}_pseudo_pred_img.png")
            oof_img_path = os.path.join(CFG["OUTPUT_DIR"], "imgs", f"fold{fold}_tta_pseudo_pred_img.png")
            pseudo_dataset = VCID_Dataset(CFG, pseudo_dirs, surface_list, mode="pseudo", transform=valid_transforms, oof_img_path=oof_img_path)
            pseudo_loader = DataLoader(pseudo_dataset, batch_size=CFG["batch_size"], shuffle = False,
                                        num_workers = CFG["num_workers"], pin_memory = True)
            ...
            for epoch in range(1, CFG["n_epoch"] + 1):
                epochs_ = epoch + CFG["n_epoch"] * slice_idx
                LOGGER.info(f'- epoch:{epochs_} -')
                train_loss_avg = train_fn(pseudo_loader, model, criterion, epochs_ ,optimizer, scheduler, CFG)
                valid_targets, valid_preds, valid_grid_idx, valid_loss_avg = valid_fn(model, valid_loader, CFG, criterion)
                # valid_targets, valid_preds, valid_grid_idx, valid_loss_avg = valid_fn(model, pseudo_loader, CFG, criterion)# pseudo data check用
                ...
                cv2.imwrite(os.path.join(CFG["OUTPUT_DIR"], "imgs", f"check_pseudo_pred_img.png"), valid_preds_img*255)
                cv2.imwrite(os.path.join(CFG["OUTPUT_DIR"], "imgs", f"check_pseudo_pred_binary.png"), valid_preds_binary*255)
                cv2.imwrite(os.path.join(CFG["OUTPUT_DIR"], "imgs", f"check_pseudo_target_img.png"), valid_targets_img*255)
                ...
                if auc > best_auc:
                    ...
                    # save oof
                    # save_and_plot_oof("auc", fold, slice_idx, valid_preds_img, valid_targets_img, valid_preds_binary, CFG)
                    cv2.imwrite(os.path.join(CFG["OUTPUT_DIR"], "imgs", f"fold{fold}_auc_slice{slice_idx}_pseudo_pred_img.png"), valid_preds_img*255)
            if valid_slice_ave is None:
                valid_slice_ave = valid_preds_img
            else:
                valid_slice_ave += valid_preds_img
        ...
        best_threshold_list.append(best_threshold)
        best_epoch_list.append(best_epoch)
        del model, pseudo_loader, pseudo_dataset, valid_loader, valid_dataset, valid_preds_img, valid_targets_img, valid_preds_binary
        gc.collect()
        torch.cuda.empty_cache()
...
if __name__=="__main__":
    ...
    for _ in range(CFG["pseudo_train_num"]):
        LOGGER.info(f"pseudo training {_+1} times")
        best_score_list, best_threshold_list, best_epoch_list = pseudo_training_loop(CFG)
