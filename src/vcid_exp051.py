...
CFG["SUMMARY"] = "ex051: subtask-class. model:efficientnetb6, img size 512, not channel shuffle, bcewithlogits weight 0.2-0.2"
...
def logging_metrics_epoch(CFG, fold, epoch, slice_idx,train_loss_avg, valid_loss_avg, score, threshold, auc_score, auc_class):
    wandb.log({f"train/fold{fold}": train_loss_avg,
                ...
                f"auc_claa/fold{fold}":auc_class,
...
def criterion(preds_class, targets_class, preds_mask, targets_mask):
    return BCE_Class(preds_class, targets_class) + BCE_Mask(preds_mask, targets_mask)
